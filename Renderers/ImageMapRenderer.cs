    ...
    public class ImageMapRenderer : Renderer, IDisposable, ICapturingRenderer
    {
        ...
        public void EnsureRenderSize(int width, int height)
        {
            if (RenderTarget is null || RenderTarget.Width < width || RenderTarget.Height < height)
            {
                ...
                RenderTarget = new(App.Instance.GraphicsDevice, width, height);
        ...
        public override void DrawTexture(Texture2D texture, Vector2 worldPos, Microsoft.Xna.Framework.Rectangle? source = null, Vector2? worldSize = null, Color? color = null, Vector2? scaleOverride = null)
        {
            ...
            if (Capturing)
            {
                App.SpriteBatch.Draw(texture, TransformVector(worldPos), source, color ?? Color.White, 0f, Vector2.Zero, scale * Scale, SpriteEffects.None, 0);
                return;
            }
            ...
            var prevState = App.SpriteBatch.GetState();
            App.SpriteBatch.End();
            BeginCapture(worldPos, new(captureWidth, captureHeight));
            App.SpriteBatch.Begin(samplerState: SamplerState.PointClamp);
            App.SpriteBatch.Draw(texture, Vector2.Zero, source, color ?? Color.White, 0f, Vector2.Zero, scaleOverride ?? scale * Scale, SpriteEffects.None, 0);
            App.SpriteBatch.End();
            EndCapture();
            App.SpriteBatch.Begin(prevState);
        }
        ...
        public JsonObject Finish()
        {
            ...
            return new()
            {
                ...
                ["layers"] = new JsonArray(App.Layers.Select(l => new JsonObject 
                {
                    ["id"] = l.Id,
                    ["name"] = l.Name,
                    ["visible"] = l.Visible,
