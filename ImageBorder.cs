...
public static class ImageBorder {
    ...
    public static bool GetEmptySides(Image<Rgba32> image, out int top, out int bottom, out int left, out int right) {
        ...
        for (var j = 0; j < image.Height && top < 0; j++) {
        ...
        for (var j = image.Height - 1; j >= 0 && bottom < 0; j--) {
            var row = image.DangerousGetPixelRowMemory(j).Span;
            for (var i = 0; i < image.Width; i++)
                if (row[i].A > 0) {
    ...
    public static Image<Rgba32> Validate(Image<Rgba32> image, out Point posDiff, int borderSize) {
        ...
        if (!GetEmptySides(image, out var top, out var bottom, out var left, out var right))
            return image;

        posDiff.X = borderSize - left;
        posDiff.Y = borderSize - top;

        var newWidth = borderSize - left + image.Width + (borderSize - right);
        var newHeight = borderSize - top + image.Height + (borderSize - bottom);
