...
public static class Interface {
    ...
    public static void BeginLayout(GameTime gt) {
        _renderer.BeginLayout(gt);
    }
    ...
    public static void Initialize() {
        ...
        var fontPath = Path.Combine(App.AppLocation, "Content/Font/MapleMonoNormalNL-NF-CN-Regular.ttf");
        ...
        if (File.Exists(fontPath))
            ...
            unsafe {
                ...
                // 转换为包装器指针以便方便操作
                var fontConfig = new ImFontConfigPtr(configPtr);
    ...
    public static void Draw() {
        ...
        var windowHeight = viewport.Size.Y;
        var windowWidth = viewport.Size.X;

        // --- 1. 设置窗口的位置和大小 ---
        // 位置：X = 总宽 - 面板宽, Y = 0 (右上角)
        ImGui.SetNextWindowPos(new Vector2(windowWidth - 400f - 8, 8));
        // 大小：宽 = SidePanelWidth, 高 = 窗口总高
        ImGui.SetNextWindowSize(new Vector2(400f, windowHeight - 16));
        ...
        if (ImGui.Begin("SidePanel", flags)) {
            ...
            if (ImGui.CollapsingHeader("视图控制", ImGuiTreeNodeFlags.DefaultOpen))
                if (ImGui.Button("重置相机")) {
                    App.WorldCamera.Position =
                        new Microsoft.Xna.Framework.Vector2(-viewport.Size.X / 2f, -viewport.Size.Y / 2f);
            ...
            if (ImGui.Button("Capture")) {
                var texture = CaptureSystem.Capture();
                var directory = Path.GetDirectoryName(App.AppLocation);

                try {
                    if (!string.IsNullOrEmpty(directory)) Directory.CreateDirectory(directory);

                    using (Stream stream = File.Create(Path.Combine(App.AppLocation, "Map.png"))) {
                    ...
                    Console.WriteLine("Error: Saving Captured Image: " + e.Message);
                } finally {
                    texture.Dispose();
                }
            }
                ...
                if (ImGui.Button("删除选中的对象") || InputHandler.DeleteEntity.Pressed)
                    foreach (var entity in Map.SelectedEntities) {
                        Console.WriteLine("Remove Entity: " +
                                          (entity.TryGet<Identifier>(out var id) ? id.Name : "Unknown"));
